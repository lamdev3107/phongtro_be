...
import { genRefreshToken } from "../utils/jwt";
...
const insertService = async () => {
  try {
    ...
    dataBody.forEach((cate) => {
      cate.body.forEach(async (item, index) => {
        let postId = v4();
    ...
    return {
      success: true,
      message: "Insert dữ liệu thành công",
    };
  } catch (error) {
    throw error;
  }
};
const createPricesAndAreas = async () => {
  try {
    ...
    return {
      success: true,
      message: "Insert giá và diện tích thành công",
    };
  } catch (err) {
    throw err;
  }
...
export { createPricesAndAreas, insertService };
